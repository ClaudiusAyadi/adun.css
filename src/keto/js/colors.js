...
for (const color in colors) {
	...
	variants[`${color}-ultra-light`] = rgbToHex(variant.tint(90).rgb);
	variants[`${color}-light`] = rgbToHex(variant.tint(75).rgb);
	variants[`${color}-medium`] = rgbToHex(variant.tint(50).rgb);
	variants[`${color}-dark`] = rgbToHex(variant.shade(30).rgb);
	variants[`${color}-ultra-dark`] = rgbToHex(variant.shade(50).rgb);
}
...
fs.readFile("src/scss/abstracts/_workers.scss", "utf8", (err, data) => {
	...
	fs.writeFile("src/scss/abstracts/_workers.scss", newColorMap, (writeErr) => {
		if (writeErr) {
